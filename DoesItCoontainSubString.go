...
func processChunk(filePath string, chunkStart, chunkSize int64, substring string, results *[]string, wg *sync.WaitGroup, progress *int64, bar *progressbar.ProgressBar, resultsMutex *sync.Mutex) {
	defer wg.Done()
	...
	for _, line := range lines {
		...
		if line != "" && strings.Contains(line, substring) {
			chunkResults = append(chunkResults, line)
		}
	}
...
func main() {
	...
	for _, chunk := range chunks {
		...
		go func(chunkStart, chunkSize int64) {
			defer func() { <-sem }()
			processChunk(passwordListFile, chunkStart, chunkSize, substring, &results, &wg, &progress, bar, &resultsMutex)
		}(chunk[0], chunk[1])
	}
	...
	fmt.Printf("Passwords containing '%s':\n", substring)
